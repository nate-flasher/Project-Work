...
def findBlockingMoves(board,player,compareList):
    newList=[]
    if player=="b":
        opposingPlayer=="r"
        opposingPlayerList=listValidMoves(board,opposingPlayer)+listSingleJumps(board,player)+listMultipleJumps(board,player,jumpsList)+findCrownRowMovesOrJumps(board,player,jumpsList)+findCrownRowMovesOrJumps(board,player,movesList)
        for move in opposingPlayerList:
            for move1 in compareList:
                if move[-2:]==move1[-2:]:
                    newList.append(move1)
    else:
        opposingPlayer="b"
        opposingPlayerList=listValidMoves(board,opposingPlayer)
        for move in opposingPlayerList:
            for move1 in compareList:
                if move[-2:]==move1[-2:]:
                    newList.append(move1)
...
def getValidMove(board,player):
    ...
    while True:
        if crowningJumps != []:
            blockList=findBlockingMoves(board,player,crowningJumps)
            if blockList != []:
                return blockList[random.randrange(0,len(blockList))]
