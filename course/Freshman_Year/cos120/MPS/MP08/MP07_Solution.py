...
def win(board):
    rmove=P1.listValidMoves(board,"r")
    rjump=P1.listSingleJumps(board,"r")
    rjump=P1.listMultipleJumps(board,"r",rjump)
    bmove=P2.listValidMoves(board,"b")
    bjump=P2.listSingleJumps(board,"b")
    bjump=P2.listMultipleJumps(board,"b",bjump)
...
def checkersMain(inFileName):
    ...
    if player=="b":
        ...
        playerColor="red"
    gameOver=False
    move = P1.getValidMove(board,player) #get the first move
    while move.lower() != "quit" and not gameOver:   #Start alternate play
        ...
        if not gameOver:
            move = P2.getValidMove(board,player)
    print("Game over, man!!!")
    if move.lower() != "quit":
