...
void Board::run() {
	...
	humans[pos] = new Human(row, col, false, this, false, false, -1, -1); 
    }
    ...
    for( int i=0; i<numHumans; ++i ) {
	    ...
	    if( isNextTo(humans[i], humans[j]) ){
		    ...
		    humans[j]->setInfected();
                if(humans[i]->isChasing){
                    humans[i]->stopChasing();
                }
		} else if ( humans[j]->isInfected() && humans[i]->isInfected()==false ) {
		    humans[i]->setInfected();
                if(humans[j]->isChasing){
                    humans[j]->stopChasing();
                }
		}
	    }
	}
