 ...
 *
 * Conio implements the text functionality of Borland's console I/O. That
 * includes cursor positioning, setting foreground/background colors,
 * setting text styles, resetting text attributes, and clearing the screen.
 *
...
/** \namespace
 *   The conio functions are not part of a class. They are just a set of
 *   functions collected within the conio namespace.
 */
namespace conio {

    /** \brief Positions cursor to the specified row, col location.
     *
     * Gotoxy will position the cursor at the specified row,col location.
     * The upper left corner has the coordinates 1,1. Note: this is
     * row,col, not x,y (col,row) as is often done.
     *
     * \param row The row coordinate (1-based).
     * \param col The column coordinate (1-based).
     * \return Returns a string containing the escape sequence to send to
     *     the screen.
     * gotoRowCol = CSI r;c
     */
