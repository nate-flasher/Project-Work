...
void displaySimInfo(conio::Color defaultTextColor, Register register1, Register register2,
		    deque<Customer> customerQ, int minute);
void displayRegisterInfo(conio::Color defaultTextColor, int row,
			 Register curRegister, int numRegister);
void displayCustServed(int totalServed);
...
int main() {
	...
	addCustomersToQueue(numNewCustomers, &customerQ, minute,
			    customerNumber);
	customerNumber += numNewCustomers;

	getNextCustomer(&register1, &currentCustomer1, &customerQ);
        if(minute - customerQ.front().arrivalTime > 10){
                getNextCustomer(&register2, &currentCustomer2, &customerQ);
                reg2open = true;
        }
        
        else if(minute - customerQ.front().arrivalTime > 5 && reg2open == true){
                getNextCustomer(&register2, &currentCustomer2, &customerQ);
                reg2open = true;
        }
	...
	displaySimInfo(defaultTextColor, register1, register2, customerQ, minute);
		    ...
		    deque<Customer> customerQ, int minute)
{
    ...
    displayRegisterInfo(defaultTextColor, 3, register1, 1);
   
    displayRegisterInfo(defaultTextColor, 4, register2, 2);


    // Show queue with the people in the queue, including highlighting to
    // show how long they have waited.
    printQueue(customerQ, minute, defaultTextColor, 5);
    printLegend(defaultTextColor, 10);
}

void displayCustServed(int totalServed) {
    cout << conio::gotoRowCol(6, 1) << "# customers finished: "
	 << totalServed;
